// The route line and the story cast. Used by the seed script and by the integration tests,
// so the tests run against exactly the data the demo uses.
...
export const CAST = {
  nusrat: { name: 'Nusrat', email: 'nusrat@dhakatesla.test' },
  rafiq: { name: 'Rafiq', email: 'rafiq@dhakatesla.test' },
  shirin: { name: 'Shirin', email: 'shirin@dhakatesla.test' },
  jashim: { name: 'Jashim', email: 'jashim@dhakatesla.test' },
} as const;

export const BULLET = { name: 'Bullet', plate: 'DHAKA-TESLA-11', capacity: 3 };

const PASSENGERS = [CAST.nusrat, CAST.rafiq, CAST.shirin];
const STARTING_WALLET_POISHA = 50_000; // ৳500

// Upserts only, so it is safe to run on every container start.
export async function seedDemoData(prisma: PrismaClient) {
  ...
  const jashim = await prisma.user.upsert({
    where: { email: CAST.jashim.email },
    update: {},
    create: { ...CAST.jashim, passwordHash, role: Role.DRIVER },
  });
